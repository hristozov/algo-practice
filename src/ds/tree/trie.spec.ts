...
describe("Trie", () => {
  describe("#add", () => {
    it("add to an empty trie", () => {
      expect(new Trie().add("cat", 15).list()).to.eql({ cat: 15 });
    });
    ...
    it("updates existing values", () => {
      expect(new Trie().add("bar", 1).add("bar", 15).list()).to.eql({
        bar: 15,
      });
    });

    it("works with prefixes", () => {
      expect(new Trie().add("b", 2).add("bar", 4).add("ba", 3).list()).to.eql({
        b: 2,
        ba: 3,
        bar: 4,
      });
  ...
  describe("#remove", () => {
    it("works with a trie with a single value", () => {
      expect(new Trie().add("bar", 2).remove("bar").list()).to.eql({});
    });
    ...
    it("works with prefixes", () => {
      const trie = new Trie().add("b", 2).add("bar", 4).add("ba", 3);
